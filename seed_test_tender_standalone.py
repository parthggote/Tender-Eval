#!/usr/bin/env python3
"""
Standalone seed script to create "Test_Tender" with complete mock data for every agency.
...
from sqlalchemy import create_engine, select, Column, String, DateTime, Float, Integer, Boolean, Text, ForeignKey
...
DATABASE_URL = os.getenv(
    ...
    "postgresql+psycopg://postgres.yqnstesksstgpselnzts:8485020909kit@aws-1-ap-northeast-1.pooler.supabase.com:5432/postgres"
)

Base = declarative_base()


# Simplified model definitions (just what we need for seeding)
...
class Tender(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    agency_id = Column(UUID(as_uuid=True), ForeignKey("agency_workspace.id"))
...
class CriterionExtractionRun(Base):
    ...
    disagreements = Column(Integer, default=0)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
...
class CriterionEvaluation(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    evaluation_run_id = Column(UUID(as_uuid=True), ForeignKey("evaluation_run.id"))
    bidder_id = Column(UUID(as_uuid=True), ForeignKey("bidder.id"))
...
def create_test_tender_for_agency(db, agency):
    ...
    # Check if Test_Tender already exists
    existing = db.execute(
        select(Tender).where(
    ...
    for bidder_data in MOCK_BIDDERS:
        ...
        for crit_idx, (verdict, reason, confidence) in bidder_data["evaluations"].items():
            ...
            # Create review cases for FAIL and NEEDS_REVIEW
...
def main():
    ...
    try:
        ...
        for agency in agencies:
            result = create_test_tender_for_agency(db, agency)
            if result:
